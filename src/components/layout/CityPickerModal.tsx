...
//       setIsSearching(true);
//       try {
//         const { data } = await apiClient.get(`/locations/search/?q=${search}`);
...
//     if (pincode) {
//       // Has a real pincode (from geolocation or search) — verify with API
//       setSelectedPincode(pincode);
//       try {
//         const { data } = await apiClient.get(`/accounts/pincode-resolve/?q=${pincode}`);
//         // ✅ Store whatever the API returns (true or false)
//         setIsServiceAvailable(data.serviceable ?? true);
//       } catch {
//         setIsServiceAvailable(true);
//       }
//     } else {
//       // ✅ No pincode (popular city card) — always true, always stored as true
//       setIsServiceAvailable(true);
//     }
...
//           await handleSelect(
//             addr.city || addr.town || addr.village || 'Delhi',
//             addr.state || '',
//             addr.postcode || null
//           );
//         } catch {
...
//                 {isSearching ? (
//                   <div className="p-6 flex items-center justify-center gap-3 text-gray-500 font-medium">
//                     <Loader2 className="animate-spin text-[#1DB8A0]" size={20} />
//                     Searching...
//                   </div>
...
//                       <button
//                         key={idx}
//                         onClick={() => handleSelect(result.city, result.state, result.pincode || null)}
//                         className="w-full flex items-center gap-4 p-4 hover:bg-emerald-50 text-left border-b border-gray-50 transition-colors"
//                       >
//                         <div className="bg-white p-2 rounded-full shadow-sm">
//                           <MapPin size={18} className="text-[#1DB8A0]" />
//                         </div>
//                         <div>
//                           <p className="font-bold text-gray-800">{result.display}</p>
//                           {result.type === 'pincode' && (
//                             <p className="text-xs text-gray-500 font-medium mt-0.5">Pincode: {result.pincode}</p>
//                           )}
//                         </div>
//                       </button>
//                     ))}
//                   </div>
//                 ) : (
//                   <div className="p-6 text-center text-gray-500 font-medium bg-gray-50">
//                     No results found for "{search}"
//                   </div>
//                 )}

//                 {/* Keep Detect Location Button pinned at the bottom */}
//                 <button
//                   onClick={handleDetectLocation}
//                   className="w-full flex items-center justify-center gap-3 p-4 bg-gray-50 border-t border-gray-100 hover:bg-gray-100 text-[#1DB8A0] font-black transition-colors"
//                 >
...
//                   ))}
//                 </div>
//                 <button onClick={() => setViewAll(false)} className="text-xs font-black text-[#1DB8A0] hover:underline">
//                   Back to Popular
//                 </button>
//               </div>
...
//                     className="text-left text-sm font-bold text-gray-600 hover:text-[#1DB8A0] transition-colors flex items-center gap-2"
//                   >
...
import { X, Search, Navigation, Loader2, Globe, MapPin } from 'lucide-react';
import { useCityContext } from '../../context/CityContext';
import apiClient from '../../api/client/apiClient';

// ============================================================================
// SVG ICONS DICTIONARY (Kept exactly as you designed them)
// ============================================================================
const CITY_ICONS: Record<string, React.FC<{ size?: number; className?: string }>> = {
  Delhi: ({ size = 64, className = '' }) => (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none" className={className}>
...
const CityPickerModal: React.FC = () => {
  ...
  // 1. Fetch Real Cities from Backend on mount
  useEffect(() => {
    if (isCityModalOpen && dbCities.length === 0) {
      apiClient.get('/locations/cities/')
        .then(res => {
          console.log("Loaded cities from API:", res.data);
          setDbCities(res.data.results || []);
  ...
  useEffect(() => {
    ...
    }, 400); // Wait 400ms before hitting the API

    return () => clearTimeout(delayDebounceFn);
  }, [search]);

  // 3. Derived State for City Grids
  const popularCities = dbCities.filter(c => c.is_popular);
  
  const filteredAlphabeticalCities = useMemo(() => {
    if (!activeLetter) return dbCities;
    return dbCities.filter(c => c.name.charAt(0).toUpperCase() === activeLetter);
  }, [dbCities, activeLetter]);


  // ============================================================================
  // CORE SELECTION LOGIC
  // ============================================================================
  const handleSelect = async (cityName: string, stateName: string, specificPincode: string | null = null) => {
    setSelectedCity(cityName);
    setSelectedState(stateName);

    // 🛡️ smart Pincode Resolution
    let finalPincode = specificPincode;
    
    // If they clicked a general city card instead of a specific pincode result, fetch the default_pincode!
    if (!finalPincode) {
      const matchedCity = dbCities.find(c => c.name === cityName);
      if (matchedCity && matchedCity.default_pincode) {
        finalPincode = matchedCity.default_pincode;
      }
    }

    if (finalPincode) {
      setSelectedPincode(finalPincode);
      try {
        // Resolve using your existing account endpoint or assume true since it came from our DB
        const { data } = await apiClient.get(`/accounts/pincode-resolve/?q=${finalPincode}`);
        const isAvail = data.serviceable === true || data.serviceable === 'true';
        setIsServiceAvailable(isAvail);
      } catch {
        // If resolution fails but we got this pincode from our locations API, it is serviceable!
        setIsServiceAvailable(true);
      }
    } else {
      // Complete fallback - No pincode known
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="p-8 overflow-y-auto max-h-[85vh] custom-scrollbar">
                    ...
                    {searchResults.map((result, idx) => (
                      ...
                      >
                        ...
                        <div>
                          <p className="font-bold text-gray-800">{result.display}</p>
                          {result.type === 'pincode' && (
                            <p className="text-xs text-[#1DB8A0] font-bold mt-0.5">Pincode: {result.pincode}</p>
                          )}
                          {result.type === 'zone' && (
                            <p className="text-xs text-gray-500 font-medium mt-0.5">Area / Zone</p>
                          )}
                        </div>
                      </button>
                    ))}
          ...
          {/* Body Content (Popular or All Cities) */}
          {!viewAll ? (
            <>
              <div className="mb-12">
                <div className="flex items-center gap-2 mb-8">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-9 gap-6">
                  {popularCities.map((city) => {
                    ...
                    return (
                      <button
                        key={city.id}
                        onClick={() => handleSelect(city.name, city.state, city.default_pincode)}
                        className="flex flex-col items-center gap-3 group relative"
                      >
                        <div className={`relative w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300 border-2 ${
                          isSelected
                          ...
                          {city.icon_url ? (
                             <img src={city.icon_url} alt={city.name} className="w-10 h-10 object-contain" />
                          ) : (
                             <CityIcon size={40} />
                          )}
                        </div>
            ...
            <div className="animate-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4 flex-wrap">
                  <span className="text-xs font-bold text-gray-400">Filter by Letter:</span>
                  <button 
                    onClick={() => setActiveLetter(null)}
                    className={`px-2 py-1 text-xs font-black rounded-md transition-colors ${activeLetter === null ? 'bg-[#1DB8A0] text-white' : 'text-gray-700 hover:bg-emerald-50'}`}
                  >
                    ALL
                  </button>
                  {availableLetters.map(char => (
                    <button 
                      ...
                      className={`w-6 h-6 flex items-center justify-center text-xs font-black rounded-md transition-colors ${activeLetter === char ? 'bg-[#1DB8A0] text-white' : 'text-gray-700 hover:text-[#1DB8A0] hover:bg-emerald-50'}`}
                    >
                      {char}
                    </button>
                  ))}
                </div>
                <button onClick={() => setViewAll(false)} className="text-xs font-black text-[#1DB8A0] hover:underline whitespace-nowrap ml-4">
                  Back to Popular
                </button>
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-4 gap-x-8">
                {filteredAlphabeticalCities.map((city) => (
                  <button
                    key={city.id}
                    onClick={() => handleSelect(city.name, city.state, city.default_pincode)}
                    className="text-left text-sm font-bold text-gray-600 hover:text-[#1DB8A0] transition-colors flex items-center gap-2 group"
                  >
                    <div className={`w-2 h-2 rounded-full transition-colors ${selectedCity === city.name ? 'bg-[#1DB8A0]' : 'bg-transparent group-hover:bg-emerald-200'}`} />
                    <span className={selectedCity === city.name ? 'text-[#1DB8A0]' : ''}>
                      {city.name}
                    </span>
                  </button>
                ))}
                
                {filteredAlphabeticalCities.length === 0 && (
                  <p className="col-span-full text-sm text-gray-400 font-medium py-8">
                    No active cities start with "{activeLetter}".
                  </p>
                )}
              </div>
            </div>
        ...
        {/* Footer */}
        <div className="px-8 py-4 bg-gray-50 flex items-center gap-3 border-t border-gray-100">
          <Globe size={14} className="text-gray-400" />
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            FlipCash service is active in {dbCities.length || '500+'} cities across India
          </p>
        </div>
      </div>
    </div>
